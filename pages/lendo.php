...
<body>
    ...
    <main>
        <div class="novo">
            <a href="cadLendo.php"><button class="btn_novo"><i class="ri-add-circle-fill"></i> <p>NOVO</p></button>
            </a>
        </div>

        <div class="lista-livros">
            <?php
            $sql = "SELECT id, titulo, capa, progresso FROM tbl_lendo";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo '<div class="livro-card">';
                    echo '<a href="conteudoLendo.php?id=' . $row['id'] . '">';
                    echo '<img src="' . $row['capa'] . '" alt="Capa de ' . $row['titulo'] . '">';
                    echo '<h3>' . $row['titulo'] . '</h3>';
                    echo '<p>' . round($row['progresso'], 2) . '%</p>'; // Exibe a porcentagem
                    echo '</a>';
                    echo '</div>';
                }
            } else {
