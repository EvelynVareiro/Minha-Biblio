...
include('../includes/connect.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
        ...
        $capa_path = $_POST['capa']; // Campo hidden para capa existente
        ...
        $sql_update = "UPDATE tbl_lido SET titulo=?, autor=?, genero=?, avaliacao=?, comentario=?, capa=? WHERE id=?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("sssissi", $titulo, $autor, $genero, $avaliacao, $comentario, $capa_path, $id_editar);

        if ($stmt_update->execute()) {
            header("Location: conteudoLido.php?id=" . $id_editar); // Redireciona para a visualização
            exit();
        } else {
        ...
        <div class="container">
            <form action="" method="post">
                ...
                <section class="parte_um">
                    <div class="voltar">
                        <a href="conteudoLido.php?id=<?php echo $livro['id']; ?>""><i class="ri-arrow-left-line"></i></a>
                    </div>

                    <div class="titulo">
                        <h1>Editar o livro lido</h1>
                    </div>
                </section>
                ...
                <section class="parte_tres" style="display: flex; flex-direction: row; gap: 40px;">
                    ...
                    <div class="input">
                        ...
                        <input type="number" id="avaliacao" name="avaliacao" value="<?php echo $livro['avaliacao']; ?>">
                    </div>
                </section>
                ...
                <section class="parte_quatro">
                    <div class="input">
                        ...
                        <textarea name="comentario" id="comentario" required><?php echo $livro['comentario']; ?></textarea>
                    </div>
                </section>
